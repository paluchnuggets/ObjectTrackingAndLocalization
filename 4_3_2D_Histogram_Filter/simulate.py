...
class Simulation(object):
    ...
    Parameters:
        grid - 2d array representing the world of size (n x n)
        blur - 
        p_hit - 
        start_pos - tuple of coordinates representing the starting position in the grid (default: middle of the grid)
    """
    ...
    def get_colors(self):
        ...
        Returns list of all colors present in the grid
        """
        all_colors = []
    ...
    def show_beliefs(self, past_turn=False):
        ...
        for y, row in enumerate(self.beliefs):
            for x, belief in enumerate(row):
                self.X.append(x)
                # puts large y ABOVE small y
                self.Y.append(self.height - y - 1)
                self.P.append(5000.0 * belief)
        plt.figure()
        if past_turn:
            plt.scatter(X, Y, s=P, alpha=0.3, color="blue")
            plt.scatter([self.prev_pose[1]], [
                        self.height - self.true_pose[0] - 1], color='red', marker="*", s=200, alpha=0.3)
        plt.scatter(self.X, self.Y, s=self.P, color="blue")
        plt.scatter([self.true_pose[1]], [self.height -
                                          self.true_pose[0] - 1], color='red', marker="*", s=200)
        plt.show()
